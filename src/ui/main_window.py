...
from PySide6.QtCore import QEvent, QTimer, QCoreApplication, Qt
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def setupUi(self):
        ...
        self._setup_clickable_key_labels()
    ...
    def _extract_key_char(self, text: str) -> str:
        """从标签文本中提取按键字符，例如 'ZL q' -> 'q'"""
    ...
    def _simulate_key_press(self, key_char: str):
        """模拟按下并释放指定的键盘按键"""
        # 字符到 Qt.Key 的映射表（根据需要补充）
        key_map = {
        'q': Qt.Key_Q, 'o': Qt.Key_O, 'e': Qt.Key_E, 'u': Qt.Key_U,
        'j': Qt.Key_J, 'i': Qt.Key_I, 'k': Qt.Key_K, 'l': Qt.Key_L,
        'r': Qt.Key_R, 'y': Qt.Key_Y, 'h': Qt.Key_H, 'g': Qt.Key_G,
        'f': Qt.Key_F, 'v': Qt.Key_V, 'c': Qt.Key_C, 'b': Qt.Key_B,
        'x': Qt.Key_X, "'": Qt.Key_Apostrophe, ';': Qt.Key_Semicolon,
        ...
        # 创建按下和释放事件
        press_event = QKeyEvent(QEvent.KeyPress, key, Qt.NoModifier, key_char)
        release_event = QKeyEvent(QEvent.KeyRelease, key, Qt.NoModifier, key_char)

        # 发送事件到当前窗口，会被已有的 eventFilter 捕获
        QCoreApplication.postEvent(self, press_event)
        # 延迟 50ms 发送释放事件，确保按键被有效识别
        QTimer.singleShot(50, lambda: QCoreApplication.postEvent(self, release_event))

    def _make_label_clickable(self, label: QLabel, key_char: str):
        """让标签可点击，点击时模拟按下对应的键盘按键"""
        if not key_char:
            return
        label.setCursor(Qt.PointingHandCursor)
        # 重写 mousePressEvent
        def mouse_press_event(event):
            self._simulate_key_press(key_char)
            # 调用原始的 mousePressEvent 以保持原有行为（如果有）
            original = getattr(label, 'original_mousePressEvent', None)
            if original:
                original(event)
        label.original_mousePressEvent = label.mousePressEvent
        label.mousePressEvent = mouse_press_event

    def _setup_clickable_key_labels(self):
        """将所有显示按键映射的标签设置为可点击"""
        # 需要处理的标签 objectName 列表
        label_names = [
        'label_zl', 'label_zr', 'label_l', 'label_r',
        'label_y', 'label_x', 'label_b', 'label_a',
        'label_minus', 'label_plus', 'label_home', 'label_capture',
        'label_dpad_t', 'label_dpad_b', 'label_dpad_l', 'label_dpad_r',
        'label_lstick_press', 'label_rstick_press',
        # 可选的摇杆方向标签（如果也需要点击模拟短暂移动）
        'label_lstick_t', 'label_lstick_b', 'label_lstick_l', 'label_lstick_r',
        'label_rstick_t', 'label_rstick_b', 'label_rstick_l', 'label_rstick_r',
        ]
        for name in label_names:
            label = getattr(self, name, None)
            if label and isinstance(label, QLabel):
                text = label.text()
                key_char = self._extract_key_char(text)
                self._make_label_clickable(label, key_char)
